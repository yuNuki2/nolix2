...
describe("LifeGame", () => {
	it("長さ", () => {
		const g1 = LifeGame.generate(SIZE);
		expect(g1.next().value.cells.flat()).toHaveLength(200);
		const g2 = LifeGame.generate({ columns: 20, rows: 10 });
		expect(g2.next().value.cells.flat()).toHaveLength(200);
	});
	it("すべて1", () => {
		const g = LifeGame.generate(SIZE, { density: 1 });
	...
	it("すべて0", () => {
		const g = LifeGame.generate(SIZE, { density: 0 });
		for (const row of g.next().value.cells) {
			for (const cell of row) {
				expect(cell).toBe(0);
			}
	...
	it("ランダム", () => {
		const g1 = LifeGame.generate(SIZE, { density: 0.5 });
		const g2 = LifeGame.generate(SIZE, { density: 0.5 });
		const actual1 = g1.next().value;
		expect(actual1).toEqual(actual1);
		expect(actual1).not.toEqual(g2.next().value);
	});
	it("シード", () => {
		const g1 = LifeGame.generate(SIZE, { density: 0.5, seed: "hello" });
		const g2 = LifeGame.generate(SIZE, { density: 0.5, seed: "hello" });
		expect(g1.next().value).toEqual(g2.next().value);
		expect(g1.next().value).toEqual(g2.next().value);
	});
	it("終了する", () => {
		const g1 = LifeGame.generate(SIZE, { density: 0.5, seed: "hello" });
		console.log(Array.from(g1).length);
		expect(g1.next().done).toBeTruthy();
	});
	it("無限ループ", () => {
		const g1 = LifeGame.generate(SIZE, { density: 0.5, seed: "hello", done: false });
		let i = 0;
		do {
			g1.next();
			i++;
	...
	it.only("グライダー", () => {
		const g1 = LifeGame.generate([10, 10], {
			defaultCells: [
				[1, 3],
				[2, 3],
				[3, 3],
	...
	it("generation", () => {
		const g = LifeGame.generate(SIZE);
